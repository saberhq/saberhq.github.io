...
**Companion accent.** Carmine `--accent-2` `#A62B4D` (dark `#DE5A78`), added Aug 2026 and adopted from the Sidechain site. It is a *naming* color, not a second link color: it marks one named thing per surface so that thing reads as separate from the site's own voice, and vermillion keeps everything else. On saberhq.com that thing is **Sidechain** — the header link and the light lapping the hero button, nothing more. Rules: never a fill, never body-link color, at most one use per view, and if a surface has no such thing to name, it simply doesn't appear. Measured against vermillion, not eyeballed — 6.49:1 on paper, ΔE 17 in OKLab, holding 16.4 / 17.9 under deuteranopia / protanopia, so the two reds stay distinguishable.
