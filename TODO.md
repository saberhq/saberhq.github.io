...
## Next
- [ ] **Sidechain standalone site at saberhq.com/sidechain** — sibling, not twin: built in the
      Sidechain repo with its own stack (interactive figures, dashboards — not Hugo).
      Mechanism note: a public repo named `sidechain` with GitHub Pages enabled publishes at
      `saberhq.com/sidechain` automatically, because this user site carries the custom domain.
- [ ] Short tab names for existing posts via the `short:` front-matter param (e.g. `short: ntEmbd`)

## Later
- [ ] Podcast & Newsletter: create real content, then unhide the sections (landing design already supports them)
